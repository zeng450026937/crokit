...
#include "yealink/native_mate/converters/v8_value_converter.h"
...
bool g_is_initialized = true; // node is always initialized when binding
...
v8::Local<v8::Value> V8ValueConverter::ToV8Array(
  ...
  for (size_t i = 0; i < val->GetSize(); ++i) {
    ...
    result->Set(context, static_cast<uint32_t>(i), child_v8).ToChecked();
